...
namespace resaxs {
    namespace algorithm {
#include "gpu_saxs_v2.cl"

        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::atomic_ff_params::initialize(const std::vector<FLT_T> &qq, std::function<void()> call_set_ext_dirty)
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::atomic_ff_params::set_expansion_factor(FLT_T c)
        {
        ...
        template <typename FLT_T>
        saxs_cl_gpu_v2<FLT_T>::gpu_pt_wf_params::gpu_pt_wf_params(saxs_cl_gpu_v2<FLT_T> &owner) :
            owner_(owner),
            ff_params_(owner, std::bind(&saxs_cl_gpu_v2<FLT_T>::on_exp_factor_changed, &owner, _1)),
            rsasa_alg(rsasa_alg_factory::template create<alg_base, rsasa_dot_spheres>())
        {
            this->set_ocl_changed_event_sink(std::bind(&saxs_cl_gpu_v2<FLT_T>::on_ocl_device_changed, &owner, _1));
        }

        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::gpu_pt_wf_params::initialize(const std::vector<FLT_T> &qq, const std::vector<real4> &bodies,
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::gpu_pt_wf_params::update_rsasa(const std::vector<real4> &bodies)
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::on_exp_factor_changed(const form_factor_params<FLT_T> & ff_params)
        {
            if (this->args_set())
            {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::on_ocl_device_changed(const ocl_exec_params &ocl_params)
        {
            if (!this->args_set())
                return;
            ...
            b_distances_.move(old_queue, this->context_);
        ...
        template <typename FLT_T>
        saxs_cl_gpu_v2<FLT_T>::saxs_cl_gpu_v2() : params_(*this), n_q_(0), n_bodies_aln_(0), avg_radius_(1.62f)
        {
        ...
        template <typename FLT_T>
        saxs_params<FLT_T> & saxs_cl_gpu_v2<FLT_T>::access_params()
        {
            return params_;
        }

        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::initialize()
        {
            alg_base::initialize(params_.get_ocl_exec_params().devices_);
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::load_program()
        {
            ...
            this->build_program(gpu_saxs_v2_cl, program_, all_options.str());
            ...
            kernel_calc_distances_ = cl::Kernel(program_, "calc_distances");
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::set_args()
        {
            ...
            /// internal device buffers
            b_margin_Iqq_.init(this->context_, CL_MEM_READ_WRITE, n_q_ * n_blocks_aln);   // aligned to 4
            b_body_factors_.init(this->context_, CL_MEM_READ_WRITE, n_q_ * n_bodies_aln_);    // aligned to WF_SIZE
            b_block_sums_.init(this->context_, CL_MEM_READ_WRITE, n_q_ * n_blocks_aln * n_blocks_aln);    // aligned to 4x4
            b_distances_.init(this->context_, CL_MEM_READ_WRITE, n_bodies_aln_ * n_bodies_aln_);
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::set_kernel_args()
        {
            const auto & factors = params_.get_ff_params().factors_;
            auto workgroup_size = params_.get_ocl_exec_params().workgroup_size_;
            ...
            kernel_calc_.setArg(ka_calc_t_dists, b_distances_);
            ...
            kernel_recalc_.setArg(ka_recalc_t_dists, b_distances_);
            ...
            kernel_recalc_with_pivots_.setArg(ka_recalc_t_dists, b_distances_);
            ...
            kernel_calc_distances_.setArg(ka_calc_dist_v_bodies, b_bodies_);
            kernel_calc_distances_.setArg(ka_calc_dist_t_distances, b_distances_);
        }

        //////////////////////////////////////////////////////////////////////////
        ///  Initialize the internal buffers for tracking transactions.
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::init_xaction()
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::update_bodies(const std::vector<real4> &bodies, unsigned int &&upd_start, unsigned int &&upd_length)
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::update_bodies(const vector<real4> &bodies)
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::calc_curve(const vector<real4> &bodies, vector<FLT_T> &out_Iqq)
        {
            ...
            this->queue_.enqueueNDRangeKernel(kernel_calc_distances_, cl::NullRange,
                cl::NDRange(n_bodies_aln_, n_bodies_aln_ / workgroup_size), cl::NDRange(workgroup_size, 1));
        ...
        //
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::recalc_curve(const std::vector<real4> &bodies, unsigned int upd_start, unsigned int upd_length, std::vector<FLT_T> &out_Iqq)
        {
            ...
            if (upd_length > 0)
            {
                ...
                if (pivot_params.pivot_start_ >= 0 && (pivot_start_aln > upd_start_aln || pivot_end_aln < upd_end_aln))
                {
        ...
        template <typename FLT_T>
        template <typename U>
        inline void saxs_cl_gpu_v2<FLT_T>::copy_if_changed(Buffer<U> &from, Buffer<U> &to, bool &changed)
        {
            if (changed)
            {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::commit()
        {
        ...
        template <typename FLT_T>
        void saxs_cl_gpu_v2<FLT_T>::revert()
        {
        ...
        template class saxs_cl_gpu_v2<float>;
        template class saxs_cl_gpu_v2<double>;
