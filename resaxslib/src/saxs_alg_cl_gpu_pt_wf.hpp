///////////////////////////////////////////////////////////////////////////////
//
//              Copyright 2015-2016 Lubo Antonov
//
...
#ifndef RE_SAXS_ALG_CL_GPU_PT_WF_HPP
#define RE_SAXS_ALG_CL_GPU_PT_WF_HPP
...
namespace algorithm {
    ...
    //
    //      Specialized to float, but can be templatized for double, etc.
    //
    template <typename FLT_T>
    class saxs_cl_gpu : public i_saxs<FLT_T, cl_base<FLT_T>>
    {
        ...
        class atomic_ff_params : public form_factor_params<FLT_T>
        {
            ...
            saxs_cl_gpu & owner_;
            const std::vector<FLT_T> *qq_ = NULL;
            std::vector<FLT_T> radii_map_;      // map of atomic radii
            FLT_T avg_radius_ = (FLT_T)(1.62);           // average radius of all bodies
        ...
        public:
            atomic_ff_params(saxs_cl_gpu &owner, std::function<void(const form_factor_params<FLT_T> &)> exp_factor_changed_sink) :
                call_set_ext_dirty_([](){}), exp_factor_changed_sink_(exp_factor_changed_sink),
                owner_(owner) {}

            void initialize(const std::vector<FLT_T> &qq, const std::vector<real4> &bodies, std::function<void()> call_set_ext_dirty = [](){});
        ...
        private:
            ...
            typedef i_rsasa<FLT_T, FLT_T, rsasa_alg_base> rsasa_class;
        ...
        public:
            gpu_pt_wf_params(saxs_cl_gpu &owner);
            ...
            virtual void initialize(const std::vector<FLT_T> &qq, const std::vector<real4> &bodies,
                const std::vector<dev_id> &devices, unsigned int workgroup_size = SAXS_NV_FERMI_WF_SIZE) override;

            virtual form_factor_params<FLT_T> & get_ff_params() override
            {
                return ff_params_;
            }
            virtual implicit_water_params<FLT_T> & get_implicit_water_params() override
            {
                return *this;
            }
            virtual ocl_exec_params & get_ocl_exec_params() override
            {
                return *this;
            }
            virtual pivot_params & get_pivot_params() override
            {
    ...
    private:
        ...
        template <typename U>
        void copy_if_changed(const Buffer<U> &from, Buffer<U> &to, bool &changed);
        
    private:
        params_type params_;


        int n_q_ = 0;                           // number of q vector elements
        int n_bodies_aln_ = 0;                  // body count aligned to the wavefront size
...
#endif // RE_SAXS_ALG_CL_GPU_PT_WF_HPP
