...
const char * gpu_saxs_v2_cl = CL_STRINGIFY(
...
inline real body_distance(real4 bodyx, real4 bodyy)
{
\n#ifdef SAXS_USE_CL_LENGTH\n
    // the distance() (and length()) function is 4x slower on NVIDIA than the manual computation
    return fast_distance(bodyx, bodyy);
\n#else\n
    real4 diff = bodyx - bodyy;
    return native_sqrt(diff.x * diff.x + diff.y * diff.y + diff.z * diff.z);
\n#endif\n
}

inline real sinc_qr(real q, real r)
{
    real qr = q * r;
...
kernel __attribute__((reqd_work_group_size(GROUP_SIZE, 1, 1)))
void calc_distances(const global real4 *restrict v_bodies,          // bodies vector        - aligned to GROUP_SIZE
            global real *restrict t_buf_dist)             // internal buffer for storing the distances
{
    ...
    {
        t_buf_dist[(idy + y) * n_bodies_aln + idx] = body_distance(bodyx, y_bodies[y]);
    }
}
...
void do_calc_block(const global real *restrict v_q,
            const global real *restrict t_dist,           // distances table        - aligned to GROUP_SIZE
                                                    ...
                                                    //                      - aligned to 4x4
{
    ...
    const global real* dist_page = t_dist + idy * GROUP_SIZE * n_bodies_aln;
    ...
    for (int y = 0; y < GROUP_SIZE; ++y)
    {
        real v1 = sinc_qr(q, dist_page[y * n_bodies_aln + idx]);
                                                    ...
                                                    //                      - aligned to 4x4
{
    ...
    do_calc_block(v_q, t_dist, t_factors, n_factors, t_body_factors, v_body_surface, water_weight, item_sum, y_factors, t_buf_blocks);
}
