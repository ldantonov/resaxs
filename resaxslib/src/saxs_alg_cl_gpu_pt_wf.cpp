...
namespace algorithm {
    ...
    void saxs_cl_gpu<FLT_T>::atomic_ff_params::initialize(const std::vector<FLT_T> &qq, const std::vector<real4> &bodies, std::function<void()> call_set_ext_dirty)
    {
        if (radii_map_.empty())
            atomic_form_factors<FLT_T>::calc_radii(radii_map_);
        avg_radius_ = atomic_form_factors<FLT_T>::calc_avg_radius(bodies, radii_map_);
        atomic_form_factors<FLT_T>::generate(qq, avg_radius_, this->expansion_factor_, this->factors_);
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::atomic_ff_params::set_expansion_factor(FLT_T c)
    {
        if (this->expansion_factor_ != c)
        {
            form_factor_params<FLT_T>::set_expansion_factor(c);

            atomic_form_factors<FLT_T>::generate(*qq_, avg_radius_, c, this->factors_);
    ...
    template <typename FLT_T>
    saxs_cl_gpu<FLT_T>::gpu_pt_wf_params::gpu_pt_wf_params(saxs_cl_gpu<FLT_T> &owner) :
        owner_(owner),
        ff_params_(owner, std::bind(&saxs_cl_gpu<FLT_T>::on_exp_factor_changed, &owner, _1)),
        rsasa_alg(rsasa_alg_factory::template create<rsasa_alg_base, rsasa_dot_spheres>())
    {
        this->set_ocl_changed_event_sink(std::bind(&saxs_cl_gpu<FLT_T>::on_ocl_device_changed, &owner, _1));
    }

    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::gpu_pt_wf_params::initialize(const std::vector<FLT_T> &qq, const std::vector<real4> &bodies,
        const std::vector<dev_id> &devices, unsigned int workgroup_size)
    {
        saxs_params<FLT_T>::initialize(qq, bodies, devices, workgroup_size);
        ff_params_.initialize(qq, bodies, std::bind(&gpu_pt_wf_params::on_dirty_set, this));
    }

    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::gpu_pt_wf_params::update_rsasa(const std::vector<real4> &bodies)
    {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::on_exp_factor_changed(const form_factor_params<FLT_T> & ff_params)
    {
        if (this->args_set())
        {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::on_ocl_device_changed(const ocl_exec_params &ocl_params)
    {
        if (!this->args_set())
            return;
    ...
    template <typename FLT_T>
    saxs_cl_gpu<FLT_T>::saxs_cl_gpu() : params_(*this)
    {
    }

    template <typename FLT_T>
    saxs_params<FLT_T> & saxs_cl_gpu<FLT_T>::access_params()
    {
        return params_;
    }

    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::initialize()
    {
        alg_base::initialize(params_.get_ocl_exec_params().devices_);
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::load_program()
    {
        ...
        this->build_program(gpu_saxs_pt_wf_cl, program_, all_options.str());
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::set_args()
    {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::set_kernel_args()
    {
        const auto & factors = params_.get_ff_params().factors_;
        auto workgroup_size = params_.get_ocl_exec_params().workgroup_size_;
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::init_xaction()
    {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::update_bodies(const std::vector<real4> &bodies, unsigned int &&upd_start, unsigned int &&upd_length)
    {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::update_bodies(const vector<real4> &bodies)
    {
    ...
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::calc_curve(const vector<real4> &bodies, vector<FLT_T> &out_Iqq)
    {
    ...
    //
    template <typename FLT_T>
    void saxs_cl_gpu<FLT_T>::recalc_curve(const std::vector<real4> &bodies, unsigned int upd_start, unsigned int upd_length, std::vector<FLT_T> &out_Iqq)
    {
        ...
        if (params_.is_dirty())
        {
            ...
            const auto &ff_params = params_.get_ff_params();
            if (ff_params.is_dirty())
            {
                b_factors_.write_from(ff_params.factors_, this->queue_);
                changed_buffers.factors = true;
            ...
            const auto & water_params = params_.get_implicit_water_params();
    ...
    //////////////////////////////////////////////////////////////////////////
    ///  Copies the from buffer into the to buffer if the changed flag is true.
    template <typename FLT_T>
    template <typename U>
    inline void saxs_cl_gpu<FLT_T>::copy_if_changed(const Buffer<U> &from, Buffer<U> &to, bool &changed)
    {
        if (changed)
        {
    ...
    template class saxs_cl_gpu<float>;
    template class saxs_cl_gpu<double>;
