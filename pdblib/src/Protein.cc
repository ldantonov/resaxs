...
namespace dsrpdb {
  ...
  unsigned int Protein::residue_offset(Residue::Index i) const {
    unsigned int cur= (unsigned int)(residues_.size());
    if (!residues_.empty()){
	...
	do {
      ...
      if (residues_[cur].index() != i) cur= (unsigned int)(residues_.size());
    }
    return cur;
  }

  unsigned int Protein::residue_offset_of_atom_index(Atom::Index index) const {
    for (int i= (unsigned int)(residues_.size())-1; i >=0; --i) {
      if (residues_[i].min_atom_index() <= index) return i;
    }
    return (unsigned int)(residues_.size());
  }
